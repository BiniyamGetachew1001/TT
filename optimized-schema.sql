-- =============================================
-- Optimized Database Schema for Content Platform
-- =============================================
...
-- Content Categories table (shared across content types)
...
-- Content policies (shared across content types)
-- Book Summaries
...
GRANT SELECT ON public.book_summaries TO authenticated, anon;
GRANT SELECT ON public.business_plans TO authenticated, anon;
...
VALUES 
  ...
  ('Zero to One', 'Peter Thiel', 'Notes on Startups, or How to Build the Future.', 
   (SELECT id FROM public.categories WHERE name = 'Entrepreneurship'), 
   'https://m.media-amazon.com/images/I/71Xygne8+qL._AC_UF1000,1000_QL80_.jpg', '18 min', 0, true),
  
  ('Deep Work', 'Cal Newport', 'Rules for Focused Success in a Distracted World.', 
   (SELECT id FROM public.categories WHERE name = 'Personal Development'), 
   'https://m.media-amazon.com/images/I/81ZZn3D9PtL._AC_UF1000,1000_QL80_.jpg', '25 min', 9.99, false),
  
  ('The Lean Startup', 'Eric Ries', 'How Today''s Entrepreneurs Use Continuous Innovation to Create Radically Successful Businesses.', 
   (SELECT id FROM public.categories WHERE name = 'Entrepreneurship'), 
   'https://m.media-amazon.com/images/I/81-QB7nDh4L._AC_UF1000,1000_QL80_.jpg', '22 min', 0, true)
...
VALUES 
  ...
  ('Tech Startup Business Plan', 'Comprehensive plan for launching a tech startup.', 
   'Technology', (SELECT id FROM public.categories WHERE name = 'Technology'), 
   'https://img.freepik.com/free-vector/gradient-technology-instagram-post-template_23-2149651738.jpg', 39.99, false),
  
  ('Restaurant Business Plan', 'Complete guide to opening a restaurant.', 
   'Food & Beverage', (SELECT id FROM public.categories WHERE name = 'Food & Beverage'), 
   'https://img.freepik.com/free-vector/restaurant-instagram-stories-template_23-2148962553.jpg', 24.99, false)
ON CONFLICT DO NOTHING;
