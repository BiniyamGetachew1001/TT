...
export const createBlogPost = async (blogPost: Omit<BlogPost, 'id' | 'created_at' | 'updated_at'>) => {
  try {
    ...
    if (coverImage && coverImage.startsWith('data:')) {
      const fileName = `${Date.now()}-${blogPost.title.replace(/\s+/g, '-').toLowerCase()}.jpg`;
    ...
    const { data, error } = await adminSupabase
      ...
      .insert([{
        ...
        is_free: blogPost.is_free || false,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      }])
      .select()
...
export const updateBlogPost = async (id: string, blogPost: Partial<BlogPost>) => {
  try {
    ...
    let coverImage = blogPost.cover_image;
    if (coverImage && coverImage.startsWith('data:')) {
      const fileName = `${Date.now()}-${blogPost.title?.replace(/\s+/g, '-').toLowerCase() || id}.jpg`;
      const { data, error } = await supabase.storage
        .from('blog-covers')
        .upload(fileName, coverImage, {
          contentType: 'image/jpeg',
    ...
    const { data, error } = await adminSupabase
      ...
      .update({
        ...blogPost,
        cover_image: coverImage,
        is_free: blogPost.is_free !== undefined ? blogPost.is_free : false,
        updated_at: new Date().toISOString()
      })
      .eq('id', id)
      .select()
